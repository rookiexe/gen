...
client.once('ready', () => {
    ...
    client.user.setActivity(`${config.prefix}yardım • ${client.user.username.toUpperCase()}`, { type: "LISTENING" }); // Set the bot's activity status
...
client.on('message', (message) => {
	...
	if (config.command.notfound_message === true && !client.commands.has(command)) {
        return message.channel.send(
            ...
            .setDescription(`Maalesef \`${command}\` komutunu bulamıyorum!`)
